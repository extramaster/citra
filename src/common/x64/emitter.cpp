...
void XEmitter::RET_FAST()  {
    Write8(0xF3);    //two-byte return (rep ret) - recommended by AMD optimization manual for the case of jumping to a ret
    Write8(0xC3);
}
...
void XEmitter::PAUSE() {
    Write8(0xF3);    //use in tight spinloops for energy saving on some cpu
    NOP();
}
void XEmitter::CLC()  {
    CheckFlags();    //clear carry
    Write8(0xF8);
}
void XEmitter::CMC()  {
    CheckFlags();    //flip carry
    Write8(0xF5);
}
void XEmitter::STC()  {
    CheckFlags();    //set carry
    Write8(0xF9);
}
...
void XEmitter::BSF(int bits, X64Reg dest, const OpArg& src) {
    WriteBitSearchType(bits,dest,src,0xBC);   // Bottom bit to top bit
}
void XEmitter::BSR(int bits, X64Reg dest, const OpArg& src) {
    WriteBitSearchType(bits,dest,src,0xBD);   // Top bit to bottom bit
}
...
void XEmitter::COMISS(X64Reg regOp, const OpArg& arg)  {
    WriteSSEOp(0x00, sseCOMIS, regOp, arg);   //weird that these should be packed
}
void XEmitter::COMISD(X64Reg regOp, const OpArg& arg)  {
    WriteSSEOp(0x66, sseCOMIS, regOp, arg);   //ordered
}
void XEmitter::UCOMISS(X64Reg regOp, const OpArg& arg) {
    WriteSSEOp(0x00, sseUCOMIS, regOp, arg);   //unordered
}
...
void XEmitter::LDDQU(X64Reg dest, const OpArg& arg)    {
    WriteSSEOp(0xF2, sseLDDQU, dest, arg);   // For integer data only
}
