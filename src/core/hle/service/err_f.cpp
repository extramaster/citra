...
static std::string GetErrType(u8 type_code) {
    ...
    case FatalErrType::Generic:        return "Generic";
    case FatalErrType::Exception:      return "Exception";
    case FatalErrType::ResultFailure:  return "ResultFailure";
    case FatalErrType::Logged:         return "Logged";
...
static std::string GetExceptionType(u8 type_code) {
    ...
    case ExceptionType::PrefetchAbort: return "Prefetch Abort";
    case ExceptionType::DataAbort:     return "Data Abort";
    case ExceptionType::Undefined:     return "Undefined Exception";
    case ExceptionType::VectorFP:      return "Vector Floating Point Exception";
...
static void LogGenericInfo(const ErrInfo::ErrInfoCommon& errinfo_common)
{
    ...
    LOG_CRITICAL(Service_ERR, "REV: 0x%08X_0x%08X", errinfo_common.rev_high, errinfo_common.rev_low);
    LOG_CRITICAL(Service_ERR, "TID: 0x%08X_0x%08X", errinfo_common.title_id_high, errinfo_common.title_id_low);
    LOG_CRITICAL(Service_ERR, "AID: 0x%08X_0x%08X", errinfo_common.app_title_id_low, errinfo_common.app_title_id_high);
...
static void ThrowFatalError(Service::Interface* self) {
    ...
    LOG_CRITICAL(Service_ERR, "Fatal error type: %s", GetErrType(errinfo->specifier).c_str());

    switch (static_cast<FatalErrType>(errinfo->specifier)) {
    case FatalErrType::Generic:
    case FatalErrType::Logged:
    {
        const auto& errtype = errinfo->generic;

        // Generic Info
        LogGenericInfo(errtype.errinfo_common);
    ...
    case FatalErrType::Exception:
    {
        ...
        for (u32 index = 0; index < errtype.exception_data.exception_context.arm_regs.size(); ++index) {
            if (index < 13) {
                LOG_DEBUG(Service_ERR, "r%u=0x%08X", index, errtype.exception_data.exception_context.arm_regs.at(index));
            } else if (index == 13) {
                LOG_CRITICAL(Service_ERR, "SP=0x%08X", errtype.exception_data.exception_context.arm_regs.at(index));
            } else if (index == 14) {
                LOG_CRITICAL(Service_ERR, "LR=0x%08X", errtype.exception_data.exception_context.arm_regs.at(index));
            } else if (index == 15) {
                LOG_CRITICAL(Service_ERR, "PC=0x%08X", errtype.exception_data.exception_context.arm_regs.at(index));
            }
        }
        LOG_CRITICAL(Service_ERR, "CPSR=0x%08X", errtype.exception_data.exception_context.cpsr);

        // Exception Info
        LOG_CRITICAL(Service_ERR, "EXCEPTION TYPE: %s", GetExceptionType(errtype.exception_data.exception_info.exception_type).c_str());
        ...
        case ExceptionType::VectorFP:
            LOG_CRITICAL(Service_ERR, "FPEXC: 0x%08X",  errtype.exception_data.exception_info.fpinst);
            LOG_CRITICAL(Service_ERR, "FINST: 0x%08X",  errtype.exception_data.exception_info.fpinst);
            LOG_CRITICAL(Service_ERR, "FINST2: 0x%08X", errtype.exception_data.exception_info.fpinst2);
            break;
        }
        LOG_CRITICAL(Service_ERR, "Datetime: %s", GetCurrentSystemTime().c_str());
        break;
